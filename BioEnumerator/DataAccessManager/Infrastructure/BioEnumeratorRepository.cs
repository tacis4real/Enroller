...
{
    internal class BioEnumeratorRepository<T> : IBioEnumeratorRepository<T> where T : class
    {


        private readonly IBioEnumeratorUoWork _context;
        private readonly DbSet<T> _dbSet;
        private readonly DbContext _dbContext;

        public BioEnumeratorRepository(BioEnumeratorUoWork uoWork)
        {
            if (uoWork == null) throw new ArgumentNullException("uoWork");
            _context = uoWork;
            _dbSet = uoWork.Context.BioEnumeratorDbContext.Set<T>();
            _dbContext = uoWork.Context.BioEnumeratorDbContext;
        }
        ...
        public T Update(T entity)
        {
            ...
            _context.Context.BioEnumeratorDbContext.Entry(entity).State = EntityState.Modified;
            return updated;
        }
        ...
        public IEnumerable<T> UpdateRange(List<T> entities)
        {
            ...
            foreach (var item in entities)
            {
                ...
                _context.Context.BioEnumeratorDbContext.Entry(item).State = EntityState.Modified;
                retVals.Add(updated);
            }
            return retVals;
        }
